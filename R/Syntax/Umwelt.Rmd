...
xnam_impenv <- paste("male + age_le_20 + age_20_40  + age_60_70 + age_gt_70 + age_40_60 +
  edu1 + edu2 + edu3 + married + city + town + village + parteinähe + impenv")
...
germany_18$impenv <-  as.factor(germany_18$impenv)
germany_18$parteinähe <-  as.factor(germany_18$parteinähe)

impact_internet = glm(as.formula(paste("internet ~ ", paste(xnam_impenv))), weights = NULL , germany_18,
                      family=binomial())

impact_blogger= glm(as.formula(paste("blogger ~ ", paste(xnam_impenv))) , weights = NULL , germany_18,
                    family=binomial())

impact_interpol = glm(as.formula(paste("interpol ~ ", paste(xnam_impenv))), weights = NULL , germany_18,
                      family=binomial())
...
psp <- design.table(Univ_18 ,~gewicht,~impenv)
int <- design.table(subset_internet,~gewicht,~impenv)
blog <- design.table(subset_blogger,~gewicht,~impenv)
inpol <- design.table(subset_interpol,~gewicht,~impenv)
...
for (i in 1:r) {
  ...
  internet.cor[i,] <- design.table(sample_internet,pseudoweights_internet[i,],~impenv)
  blogger.cor[i,] <- design.table(sample_blogger,pseudoweights_blogger[i,],~impenv)
  interpol.cor[i,] <- design.table(sample_interpol,pseudoweights_interpol[i,],~impenv)
...
internet.rak[i,] <- design.table(sample_internet,calibrationweights_internet[i,],~impenv)
blogger.rak[i,] <- design.table(sample_blogger,calibrationweights_blogger[i,],~impenv)
interpol.rak[i,] <- design.table(sample_interpol,calibrationweights_interpol[i,],~impenv)
...
names(bias) <- c("sehr_wichtig","wichtig","eher_wichtig","kaum_wichtig","nicht_wichtig","gar_nicht_wichtig","Selektion")
...
names(verz) <- c("sehr_wichtig2","wichtig2","eher_wichtig2","kaum_wichtig2","nicht_wichtig2","gar_nicht_wichtig2","Selektion2")
...
plot3 <- plotter(bias,bias$Selektion,bias$eher_wichtig,verz,verz$Selektion2,verz$eher_wichtig2,"eher_wichtig")
plot4 <- plotter(bias,bias$Selektion,bias$kaum_wichtig,verz,verz$Selektion2,verz$kaum_wichtig2,"kaum_wichtig")
plot5 <- plotter(bias,bias$Selektion,bias$nicht_wichtig,verz,verz$Selektion2,verz$nicht_wichtig2,"nicht_wichtig")
plot6 <- plotter(bias,bias$Selektion,bias$gar_nicht_wichtig,verz,verz$Selektion2,verz$gar_nicht_wichtig2,"gar_nicht_wichtig")


grid.arrange(plot1, plot2, plot3, plot4, plot5, plot6, nrow = 3, ncol = 3, top = textGrob("Pseudogewichtete Bedeutung Umwelt",gp=gpar(fontsize=20,font=1)))
...
names(bias.rak) <-  c("sehr_wichtig","wichtig","eher_wichtig","kaum_wichtig","nicht_wichtig","gar_nicht_wichtig","Selektion")
...
plot3.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$eher_wichtig,verz,verz$Selektion2,verz$eher_wichtig2,"eher_wichtig")
plot4.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$kaum_wichtig,verz,verz$Selektion2,verz$kaum_wichtig2,"kaum_wichtig")
plot5.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$nicht_wichtig,verz,verz$Selektion2,verz$nicht_wichtig2,"nicht_wichtig")
plot6.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$gar_nicht_wichtig,verz,verz$Selektion2,verz$gar_nicht_wichtig2,"gar_nicht_wichtig")

grid.arrange(plot1.rak, plot2.rak, plot3.rak, plot4.rak, plot5.rak,plot6.rak, nrow = 3, ncol = 3, top = textGrob("Calibrierte Bedeutung Umwelt",gp=gpar(fontsize=20,font=1)))
