...
rakemarg <- function(x,y){
  margins_2 <- matrix(nrow = length(variable.names(xVar)),ncol = 10)
  for (i in 1:length(variable.names(y))) {
    ...
    v7 <- table(x[variable.names(y[i])])[7]
    v8 <- table(x[variable.names(y[i])])[8]
    vec <- c(n,c,v1,v2,v3,v4,v5,v6,v7,v8)
    margins_2[i,] <-   vec 
  } 
...
ESS9DE <- ESS9DE %>% mutate(male = gndr == 1,
                            ...
                            partei = case_when(prtvede2 == 1~"Union",
                                     ...
                                     vote== 2 ~ "Nichtwähler"),
                            gesundheit = case_when(health == 1 ~ 1,
                                                   health == 2 ~ 2,
                                                   health >= 3 ~ 3))
...
#gesundheit 1 Sehr gut 2 gut 3 Fair bis Schlecht
table(ESS9DE$gesundheit)
...
xVar <- ESS9DE %>%  select(age_le_20,age_20_40,age_40_60,age_60_70,age_gt_70,
                           ...
                           city, town, village, partei)
yvar <- ESS9DE %>%  select(gesundheit) 
...
xnam_gesundheit <- paste("male + age_le_20 + age_20_40  + age_60_70 + age_gt_70  +
  edu1 + edu2  + married + city + town  + partei + gesundheit")
...
germany_18$gesundheit <-  as.factor(germany_18$gesundheit)
...
impact_internet = glm(as.formula(paste("internet ~ ", paste(xnam_gesundheit))), weights = NULL , germany_18,
                      family=binomial())

impact_blogger= glm(as.formula(paste("blogger ~ ", paste(xnam_gesundheit))) , weights = NULL , germany_18,
                    family=binomial())

impact_interpol = glm(as.formula(paste("interpol ~ ", paste(xnam_gesundheit))), weights = NULL , germany_18,
                      family=binomial())
# 
 summary(impact_internet)#
...
stargazer(impact_internet,impact_blogger,impact_interpol,
          ...
          out = "Z:/Zwischenergebnisse_Nichtwahl/Gesundheit/SAR- Annahme/SAR-Wahleintscheidung.txt",
          ...
          covariate.labels = c("Männlich (Ja)","Alter<20 vs.40-60","20-40 vs.40-60","60-70 vs.40-60","Alter>70 vs.40-60",
                               ...
                               "Großstadt vs. Dorf", "Stadt vs. Dorf","Union vs. Nichtwähler",
                               "SPD vs. Nichtwähler","Linke vs. Nichtwähler","Grüne vs. Nichtwähler",
                               "FDP vs. Nichtwähler","AFD vs. Nichtwähler","Sonstige vs. Nichtwähler",
                               "Gesundheit (gut vs. sehr gut)","Gesundheit (fair oder schlechter vs. sehr gut)","(Intercept)" ))
...
psp <- design.table(Univ_18 ,~gewicht,~gesundheit)
int <- design.table(subset_internet,~gewicht,~gesundheit)
blog <- design.table(subset_blogger,~gewicht,~gesundheit)
inpol <- design.table(subset_interpol,~gewicht,~gesundheit)
...
for (i in 1:r) {
  ...
  internet.cor[i,] <- design.table(sample_internet,pseudoweights_internet[i,],~gesundheit)
  blogger.cor[i,] <- design.table(sample_blogger,pseudoweights_blogger[i,],~gesundheit)
  interpol.cor[i,] <- design.table(sample_interpol,pseudoweights_interpol[i,],~gesundheit)
...
internet.rak[i,] <- design.table(sample_internet,calibrationweights_internet[i,],~gesundheit)
blogger.rak[i,] <- design.table(sample_blogger,calibrationweights_blogger[i,],~gesundheit)
interpol.rak[i,] <- design.table(sample_interpol,calibrationweights_interpol[i,],~gesundheit)
...
write.csv2(internet_tabelle, file = "Z:/Zwischenergebnisse_Nichtwahl/Gesundheit/Tabellen/internet_tabelle.csv", quote = FALSE, row.names = T)
write.csv2(blogger_tabelle, file = "Z:/Zwischenergebnisse_Nichtwahl/Gesundheit/Tabellen/blogger_tabelle.csv", quote = FALSE, row.names = T)
write.csv2(interpol_tabelle, file = "Z:/Zwischenergebnisse_Nichtwahl/Gesundheit/Tabellen/interpol_tabelle.csv", quote = FALSE, row.names = T)
...
names(bias) <- c("SehrGut","Gut","FairoderSchlechter","Selektion")
...
names(verz) <- c("SehrGut2","Gut2","FairoderSchlechter2","Selektion2")
verz [,1:length(table(yvar))] <- apply(verz[,1:length(table(yvar))], 2,as.numeric)

plot1 <- plotter(bias,bias$Selektion,bias$SehrGut,verz,verz$Selektion2,verz$SehrGut2,"Sehr Gut")
plot2 <- plotter(bias,bias$Selektion,bias$Gut,verz,verz$Selektion2,verz$Gut2,"Gut")
plot3 <- plotter(bias,bias$Selektion,bias$FairoderSchlechter,verz,verz$Selektion2,verz$FairoderSchlechter2,"Fair oder Schlechter")

grid.arrange(plot1, plot2, plot3, ncol = 3, top = textGrob("Pseudogewichtete Gesundheit",gp=gpar(fontsize=20,font=1)))
...
names(bias.rak) <-  c("SehrGut","Gut","FairoderSchlechter","Selektion")
bias.rak[,1:length(table(yvar))] <- apply(bias.rak[,1:length(table(yvar))], 2,as.numeric)

plot1.rak <- plotter(bias,bias$Selektion,bias$SehrGut,verz,verz$Selektion2,verz$SehrGut2,"Sehr Gut")
plot2.rak <- plotter(bias,bias$Selektion,bias$Gut,verz,verz$Selektion2,verz$Gut2,"Gut")
plot3.rak <- plotter(bias,bias$Selektion,bias$FairoderSchlechter,verz,verz$Selektion2,verz$FairoderSchlechter2,"Fair oder Schlechter")

grid.arrange(plot1.rak, plot2.rak, plot3.rak, ncol = 3, top = textGrob("Calibrierte Gesundheit",gp=gpar(fontsize=20,font=1)))
