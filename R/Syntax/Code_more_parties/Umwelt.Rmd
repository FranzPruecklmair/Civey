...
xnam_umwelt <- paste("male + age_le_20 + age_20_40  + age_60_70 + age_gt_70 +
  edu1 + edu2  + married + city + town + partei + umwelt")
...
germany_18$umwelt <-  as.factor(germany_18$umwelt)
...
impact_internet = glm(as.formula(paste("internet ~ ", paste(xnam_umwelt))), weights = NULL , germany_18,
                      family=binomial())

impact_blogger= glm(as.formula(paste("blogger ~ ", paste(xnam_umwelt))) , weights = NULL , germany_18,
                    family=binomial())

impact_interpol = glm(as.formula(paste("interpol ~ ", paste(xnam_umwelt))), weights = NULL , germany_18,
                      family=binomial())
...
stargazer(impact_internet,impact_blogger,impact_interpol,
          ...
          out = "Z:/Zwischenergebnisse_Nichtwahl/Umwelt/SAR- Annahme/SAR-Umwelt.txt",
          ...
          covariate.labels = c("Männlich (Ja)","Alter<20 vs.40-60","20-40 vs.40-60","60-70 vs.40-60","Alter>70 vs.40-60",
                               ...
                               "(Umwelt) Wichtig vs. Sehr wichtig","(Umwelt) nicht so Wichtig vs. Sehr wichtig","(Intercept)" ))
...
psp <- design.table(Univ_18 ,~gewicht,~umwelt)
int <- design.table(subset_internet,~gewicht,~umwelt)
blog <- design.table(subset_blogger,~gewicht,~umwelt)
inpol <- design.table(subset_interpol,~gewicht,~umwelt)
...
for (i in 1:r) {
  ...
  internet.cor[i,] <- design.table(sample_internet,pseudoweights_internet[i,],~umwelt)
  blogger.cor[i,] <- design.table(sample_blogger,pseudoweights_blogger[i,],~umwelt)
  interpol.cor[i,] <- design.table(sample_interpol,pseudoweights_interpol[i,],~umwelt)
...
internet.rak[i,] <- design.table(sample_internet,calibrationweights_internet[i,],~umwelt)
blogger.rak[i,] <- design.table(sample_blogger,calibrationweights_blogger[i,],~umwelt)
interpol.rak[i,] <- design.table(sample_interpol,calibrationweights_interpol[i,],~umwelt)
...
write.csv2(internet_tabelle, file = "Z:/Zwischenergebnisse_Nichtwahl/Umwelt/Tabellen/internet_tabelle.csv", quote = FALSE, row.names = T)
write.csv2(blogger_tabelle, file = "Z:/Zwischenergebnisse_Nichtwahl/Umwelt/Tabellen/blogger_tabelle.csv", quote = FALSE, row.names = T)
write.csv2(interpol_tabelle, file = "Z:/Zwischenergebnisse_Nichtwahl/Umwelt/Tabellen/interpol_tabelle.csv", quote = FALSE, row.names = T)
...
names(bias) <- c("sehr_wichtig","wichtig","nicht_so_wichtig","Selektion")
...
names(verz) <- c("sehr_wichtig2","wichtig2","nicht_so_wichtig2","Selektion2")
verz [,1:length(table(yvar))] <- apply(verz[,1:length(table(yvar))], 2,as.numeric)

plot1 <- plotter(bias,bias$Selektion,bias$sehr_wichtig,verz,verz$Selektion2,verz$sehr_wichtig2,"sehr_wichtig")
plot2 <- plotter(bias,bias$Selektion,bias$wichtig,verz,verz$Selektion2,verz$wichtig2,"wichtig")
plot3 <- plotter(bias,bias$Selektion,bias$nicht_so_wichtig,verz,verz$Selektion2,verz$nicht_so_wichtig2,"nicht_so_wichtig")



grid.arrange(plot1, plot2, plot3, ncol = 3, top = textGrob("Pseudogewichtete Bedeutung umwelt",gp=gpar(fontsize=20,font=1)))
...
names(bias.rak) <-  c("sehr_wichtig","wichtig","nicht_so_wichtig","Selektion")
bias.rak[,1:length(table(yvar))] <- apply(bias.rak[,1:length(table(yvar))], 2,as.numeric)

plot1.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$sehr_wichtig,verz,verz$Selektion2,verz$sehr_wichtig2,"sehr_wichtig")
plot2.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$wichtig,verz,verz$Selektion2,verz$wichtig2,"wichtig")
plot3.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$nicht_so_wichtig,verz,verz$Selektion2,verz$nicht_so_wichtig2,"nicht_so_wichtig")

grid.arrange(plot1.rak, plot2.rak, plot3.rak, ncol = 3, top = textGrob("Calibrierte Bedeutung umwelt",gp=gpar(fontsize=20,font=1)))
