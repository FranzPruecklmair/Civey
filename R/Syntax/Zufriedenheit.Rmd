...
ESS9DE <- ESS9DE %>% mutate(male = gndr == 1,
                            ...
                            gewicht = pspwght,
                            parteinähe = prtclede,
                            Zufr = case_when( stflife > 8  ~ "extreme satisfied",
                                              stflife == 8|stflife == 7|stflife == 6  ~ "satisfied",
                                              stflife == 5 ~ "balanced",
                                              stflife == 4|stflife == 3|stflife == 2  ~ "dissatisfied",
                                              stflife < 2  ~ "extreme dissatisfied"
                            ))
...
xnam_Zufr <- paste("male + age_le_20 + age_20_40  + age_60_70 + age_gt_70 + age_40_60 +
  edu1 + edu2 + edu3 + married + city + town + village + parteinähe + Zufr")
...
psp <- design.table(ESS9DE,~pspwght,~Zufr)
int <- design.table(INTERNET,~pspwght,~Zufr)
blog <- design.table(BLOGGER,~pspwght,~Zufr)
inpol <- design.table(INTERPOL,~pspwght,~Zufr)
...
names(bias) <- c("balanced", "dissatisfied", "extreme_dissatisfied", "extreme_satisfied", "satisfied","Selektion")
...
names(verz) <- c("balanced2", "dissatisfied2", "extreme_dissatisfied2", "extreme_satisfied2", "satisfied2","Selektion2")
verz [,1:length(table(yvar))] <- apply(verz[,1:length(table(yvar))], 2,as.numeric)

plot1 <- plotter(bias,bias$Selektion,bias$balanced,verz,verz$Selektion2,verz$balanced2,"balanced")
plot2 <- plotter(bias,bias$Selektion,bias$dissatisfied,verz,verz$Selektion2,verz$dissatisfied2,"dissatisfied")
plot3 <- plotter(bias,bias$Selektion,bias$extreme_dissatisfied,verz,verz$Selektion2,verz$extreme_dissatisfied2,"extreme_dissatisfied")
plot4 <- plotter(bias,bias$Selektion,bias$extreme_satisfied,verz,verz$Selektion2,verz$extreme_satisfied2,"extreme_satisfied")
plot5 <- plotter(bias,bias$Selektion,bias$satisfied,verz,verz$Selektion2,verz$satisfied2,"satisfied")



grid.arrange(plot1, plot2, plot3, plot4, plot5, nrow = 3, ncol = 3, top = textGrob("Pseudogewichtete Zufriedenheit",gp=gpar(fontsize=20,font=1)))
...
names(bias.rak) <- c("balanced", "dissatisfied", "extreme_dissatisfied", "extreme_satisfied", "satisfied","Selektion")
bias.rak[,1:length(table(yvar))] <- apply(bias.rak[,1:length(table(yvar))], 2,as.numeric)

plot1.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$balanced,verz,verz$Selektion2,verz$balanced2,"balanced")
plot2.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$dissatisfied,verz,verz$Selektion2,verz$dissatisfied2,"dissatisfied")
plot3.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$extreme_dissatisfied,verz,verz$Selektion2,verz$extreme_dissatisfied2,"extreme_dissatisfied")
plot4.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$extreme_satisfied,verz,verz$Selektion2,verz$extreme_satisfied2,"extreme_satisfied")
plot5.rak <- plotter(bias.rak,bias.rak$Selektion,bias.rak$satisfied,verz,verz$Selektion2,verz$satisfied2,"satisfied")



grid.arrange(plot1.rak, plot2.rak, plot3.rak, plot4.rak, plot5.rak, nrow = 3, ncol = 3, top = textGrob("Calibrierte Zufriedenheit",gp=gpar(fontsize=20,font=1)))
